...
Usage:
    python cli.py repl <path> [<path> ...]
    python cli.py index <path> [<path> ...]
    python cli.py query <name> [--depth N] [--path <path> ...]
    python cli.py lookup <name> [--path <path> ...]
    python cli.py search <prefix> [--limit N] [--path <path> ...]
    python cli.py symbols <file> [--path <path> ...]
    python cli.py opencode <name> [--path <path> ...]
"""
...
# ---------------------------------------------------------------------------
# fce module discovery — search common build output locations
# ---------------------------------------------------------------------------
...
_SEARCH_PATHS = [
    ...
    # standalone build (build_fce_standalone)
    os.path.join(os.path.dirname(__file__), "..", "..", "..", "build_fce_standalone", "bindings", "Release"),
    os.path.join(os.path.dirname(__file__), "..", "..", "..", "build_fce_standalone", "bindings", "Debug"),
    os.path.join(os.path.dirname(__file__), "..", "..", "..", "build_fce_standalone", "bindings"),
]
...
try:
    ...
    print("ERROR: cannot import 'fce' module.")
    print("Build it first:  cmake -B build && cmake --build build --config Release")
    print("Searched:", [os.path.normpath(p) for p in _SEARCH_PATHS])
    sys.exit(1)
...
def do_opencode(engine: fce.Engine, name: str) -> None:
    """Look up symbol, read source file, display code with line numbers."""
...
def main() -> None:
    ...
    # symbols: default --path to the file's directory
